...
sudo ip link add n1-d0 type dummy
...
sudo ip link set n1-d0 netns node1
...
sudo ip netns exec node1 ip addr add 10.68.0.1/32 dev n1-d0
...
sudo ip netns exec node1 ip link set up dev n1-d0
