...
use super::packet::attribute::ASSegment;
...
impl From<&Path> for sartd_proto::sart::Path {
    fn from(path: &Path) -> Self {
        let mut segs = Vec::new();
        if !path.as_sequence.is_empty() {
            segs.push(sartd_proto::sart::AsSegment {
        ...
        sartd_proto::sart::Path {
            nlri: path.prefix.to_string(),
            family: Some(sartd_proto::sart::AddressFamily::from(&path.family)),
