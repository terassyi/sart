...
use crate::error::{Error, MessageHeaderError};
...
pub enum Message {
    ...
    Open {
        ...
        capabilities: Vec<capability::Cap>,
    },
...
impl MessageBuilder {
        ...
        match self.msg_type {
            MessageType::Open => {
                let asn = if self
                    .capabilities
                    .iter()
                    .filter(|cap| matches!(cap, capability::Cap::FourOctetASNumber(_)))
                    .count()
                    == 0
                {
                    self.asn
    ...
    pub fn capability(&mut self, capability: &Capability) -> Result<&mut Self, Error> {
        if self.msg_type != MessageType::Open {
            return Err(Error::InvalidMessageField);
        }
...
pub fn validate(msg: &Message) -> Result<(), Error> {
    match &msg {
        Message::Open {
            version: _,
            as_num: _,
...
#[cfg(test)]
mod tests {
    use super::capability::Cap; // packet
    use super::Message;
    ...
    use crate::error::MessageHeaderError;
    use crate::family::{AddressFamily, Afi, Safi};
    use rstest::rstest;
    use std::net::Ipv4Addr;
