...
impl Prefix {
	pub fn decode(family: AddressFamily, add_path_enabled: bool, data: &mut BytesMut) -> Result<Self, Error> {
		...
		let p = match family.afi {
			Afi::IPv4 => {
				...
				let pref = Ipv4Net::new(addr, length).map_err(|_| Error::UpdateMessage(UpdateMessageError::InvalidNetworkField))?;
				IpNet::V4(pref)
			},
			Afi::IPv6 => {
				let mut b = [0u8; 16];
				...
				let pref = Ipv6Net::new(addr, length).map_err(|_| Error::UpdateMessage(UpdateMessageError::InvalidNetworkField))?;
				IpNet::V6(pref)
			},
		};
...
mod tests {
	...
	fn works_prefix_decode(family: AddressFamily, add_path_enabled: bool, input: Vec<u8>, expected: Prefix) {
