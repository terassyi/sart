...
pub(crate) enum AllocatorMethod {
    Bit,
}
...
impl Allocator {
    ...
    pub(crate) fn new(cidr: IpNet, method: AllocatorMethod) -> Result<Allocator, Error> {
    ...
    pub(crate) fn allocate(&mut self, addr: &IpAddr) -> Result<IpAddr, Error> {
        match self {
            Allocator::Bit(a) => a.allocate(addr),
        }
    }

    pub(crate) fn allocate_next(&mut self) -> Result<IpAddr, Error> {
        match self {
            Allocator::Bit(a) => a.allocate_next(),
        }
    }

    pub(crate) fn release(&mut self, addr: &IpAddr) -> Result<IpAddr, Error> {
        match self {
            Allocator::Bit(a) => a.release(addr),
        }
    }

    pub(crate) fn is_allocated(&self, addr: &IpAddr) -> bool {
        match self {
            Allocator::Bit(a) => a.is_allocated(addr),
        }
    }

    pub(crate) fn cidr(&self) -> &IpNet {
        match self {
