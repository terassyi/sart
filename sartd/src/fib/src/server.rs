...
use super::{kernel::KernelRtPoller, rib::RequestType};
use sartd_proto::sart::{
    fib_manager_api_server::{FibManagerApi, FibManagerApiServer},
    GetChannelRequest, GetChannelResponse, GetRoutesRequest, GetRoutesResponse, ListChannelRequest,
    ListChannelResponse,
};
...
impl FibManagerApi for Fib {
    async fn get_channel(
        ...
        if let Some(channel) = self.channels.iter().find(|&ch| ch.name.eq(name)) {
            let res = sartd_proto::sart::Channel::from(channel);
    ...
    async fn list_channel(
        ...
        let channels: Vec<sartd_proto::sart::Channel> = self
            .channels
            .iter()
            .map(sartd_proto::sart::Channel::from)
            .collect();
    ...
    async fn get_routes(
        ...
        if let Some(channel) = self.channels.iter().find(|&ch| ch.name.eq(name)) {
            match channel.list_routes() {
                Some(routes) => {
                    let routes = routes.iter().map(sartd_proto::sart::Route::from).collect();
