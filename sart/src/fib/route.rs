...
use crate::{error::Error, rpc::{connect_fib}, cmd::Format, proto::sart::GetRoutesRequest};
...
pub(crate) async fn get(endpoint: &str, format: Format, name: String) -> Result<(), Error> {
	...
	let res = client.get_routes(GetRoutesRequest{
		channel: name
	...
	match format {
		Format::Json => {},
		Format::Plain => {
			let display: Vec<DisplayedRoute> = res.get_ref().routes.iter().map(DisplayedRoute::from).collect();
