...
use crate::{cmd::Format, error::Error, rpc::connect_fib, proto::sart::{GetChannelRequest, ListChannelRequest}};
...
pub(crate) async fn get(endpoint: &str, format: Format, name: String) -> Result<(), Error> {
	...
	let res = client.get_channel(GetChannelRequest{
		name
	}).await.map_err(Error::FailedToGetResponse)?;
...
pub(crate) async fn list(endpoint: &str, format: Format) -> Result<(), Error> {
	...
	let res = client.list_channel(ListChannelRequest{}).await.map_err(Error::FailedToGetResponse)?;
