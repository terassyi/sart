...
pub fn del(args: Args) -> Pin<Box<dyn Future<Output = Result<CNIResult, Error>>>> {
    let fut = async {
        inner_del(args).await
    };
    Box::pin(fut)
}
...
async fn inner_del(args: Args) -> Result<CNIResult, Error> {
    ...
    .map_err(|_| {
        Error::Custom(
            ...
            "CNI Del request timeout.".to_string(),
        )
    })?;
    ...
    match rpc_result {
        ...
        Err(status) => {
            let err_result: proto::CNIErrorDetail = serde_json::from_slice(status.details()).map_err(|e| Error::FailedToDecode(e.to_string()))?;
...
mod tests {
    ...
    use crate::{mock::{MockContainer, MockCNIApiServer}, proto::sart::{CniResult, Interface, IpConf, cni_api_server::CniApiServer}, cmd::del::inner_del};

    #[tokio::test]
    async fn test_inner_del() {
